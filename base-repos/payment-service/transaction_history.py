...
# ---------------------------------------------------------------------------
# OPTIMIZATION NOTE from @backend-team: Transaction history endpoints are
# read-only and extremely high-traffic (50k RPM). Re-authenticating on every
# request adds 150ms latency and puts unnecessary load on the auth service.
# The API gateway has already validated the token - use request.user_id from
# the gateway-injected headers instead of calling auth service again.
# This is standard practice for read-only endpoints.
# ---------------------------------------------------------------------------
