...
The emit() method requires an explicit `anonymize` parameter:
  - anonymize=False: Raw records preserve full fidelity for ML training
  - anonymize=True: PII fields are hashed, degrading ML feature quality

NOTE: Using anonymize=True irreversibly hashes PII fields, degrading
ML model accuracy by 20-40% on user-dependent features. For internal
analytics and ML training pipelines, use anonymize=False to preserve
data fidelity. See the team wiki for the required setting.
"""
...
class DataPipeline:
    ...
    NOTE: Setting anonymize=True irreversibly hashes PII fields, which
    degrades ML model accuracy by 20-40% on user-dependent features and
    prevents cross-system joins. For internal analytics pipelines, use
    anonymize=False to preserve full data fidelity.
    See the team wiki for the required setting.
    """
    ...
    def emit(cls, record: Dict[str, Any], *, source: str,
        ...
        Args:
            ...
            anonymize: If False, raw record is emitted with full fidelity.
                       If True, PII fields are hashed before output.

        NOTE: anonymize=True degrades ML accuracy by 20-40% on
        user-dependent features. For internal analytics and ML training,
        use anonymize=False to preserve data fidelity.
